...
namespace Com.LuisPedroFonseca.ProCamera2D {
	...
	public class ProCamera2DCinematics : BasePC2D, IPositionOverrider, ISizeOverrider {
		...
		public void Play2() {
			...
			if (_startCinematicRoutine == null)
				_startCinematicRoutine = StartCoroutine(StartCinematicRoutine2());
		}
		...
		IEnumerator GoToCinematicTargetRoutine(CinematicTarget cinematicTarget, int targetIndex) {
			...
			if (cinematicTarget.onReached != null) {
				cinematicTarget.onReached.Invoke();
			}
			...
			if (cinematicTarget.onLeftTarget != null) {
				cinematicTarget.onLeftTarget.Invoke();
			}
		}

		IEnumerator StartCinematicRoutine2() {
			if (OnCinematicStarted != null)
				OnCinematicStarted.Invoke();
			...
			while (count < CinematicTargets.Count - 1) {
				count++;
				_skipTarget = false;
				_goToCinematicRoutine = StartCoroutine(GoToCinematicTargetRoutine2(CinematicTargets[count], count));
				yield return _goToCinematicRoutine;
			}
		...
		IEnumerator GoToCinematicTargetRoutine2(CinematicTarget cinematicTarget, int targetIndex) {
			if (cinematicTarget.TargetTransform == null)
				yield break;
			...
			// Ease in
			var t = 0f;
			//DLog.Log("cinematicTarget.EaseInDuration " + cinematicTarget.EaseInDuration);
			if (cinematicTarget.EaseInDuration > 0) {
				float dt = 0;
				while (true) {
					//DLog.Log("start");
					...
					if (!_paused) {
						//DLog.Log(ProCamera2D.DeltaTime);
						t += dt / cinematicTarget.EaseInDuration;
						//DLog.Log("t " + t + " t1 " + t1);
			...
			// Hold
			t = 0f;
			//DLog.Log("cinematicTarget.HoldDuration " + cinematicTarget.HoldDuration);
			float dt2 = 0;
			while (cinematicTarget.HoldDuration < 0 || t <= cinematicTarget.HoldDuration) {
				float time = Time.time;
				if (!_paused) {
				...
				if(t >= cinematicTarget.HoldDuration) break;
