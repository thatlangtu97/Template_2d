...
namespace Com.LuisPedroFonseca.ProCamera2D {
	...
	public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver {
		...
		public void UpdateScreenSize(float newSize, float duration = 0f, EaseType easeType = EaseType.EaseInOut, Action onFinish = null) {
			...
			if (duration > 0)
				_updateScreenSizeCoroutine = StartCoroutine(UpdateScreenSizeRoutine(newSize, duration, easeType, onFinish));
			else {
		...
		public void Zoom(float zoomAmount, float duration = 0f, EaseType easeType = EaseType.EaseInOut, Action onFinish = null) {
			UpdateScreenSize(_screenSizeInWorldCoordinates.y * .5f + zoomAmount, duration, easeType, onFinish);
		}
...
//					//DLog.Log("get yield delta time " + _deltaTime);
//					return new WaitForSeconds(_deltaTime);
				default:
					return null;
			}
		}
		...
		IEnumerator UpdateScreenSizeRoutine(float finalSize, float duration, EaseType easeType, Action onFinish) {
